...
step(app(c(tl),app(app(c(cons),V),VS)),V) :- value(V),value(VS).  %%%% BUG
...
*)
 ...
 (*
  ...
  NF need a generator for E, the grounding
  is obtained by mode inference.				
...
NE-simpl yes (some 40secs), NE exhausts space too quickly.
*)
