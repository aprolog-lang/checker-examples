...
subst(let(y\E2,E1),X,N) = let(y\E2',E1') :- y # E1, subst(E1,X,N) = E1', subst(E2,X,N) = E2'.
subst(inl(E),X,N) = inl(subst(E,X,N)).
subst(inr(E),X,N) = inr(subst(E,X,N)).
subst(case(E,L,R),X,N) = case(subst(E,X,N),L,R).
...
tc(G,inl(E),sumTy(TL,TR)) :- tc(G,E,TL).
tc(G,inr(E),sumTy(TL,TR)) :- tc(G,E,TR).
tc(G,case(E,x\L,y\R),T) :- exists TL. exists TR. x # G, y # G, tc(G,E,sumTy(TL,TR)), tc([(x,TL)|G],L,T), tc([(y,TR)|G],R,T).
...
step(inl(E), inl(E')) :- step(E,E').
step(inr(E), inr(E')) :- step(E,E').
step(case(inl(V),x\L,y\R), E) :- value(V), x # V, substp(L,x,V,E).
step(case(inr(V),x\L,y\R), E) :- value(V), y # V, substp(R,y,V,E).
...
exists_step(inl(E)) :- exists_step(E).
exists_step(inr(E)) :- exists_step(E).
exists_step(case(E,x\L,y\R)) :- exists_step(E).
exists_step(case(inl(V),x\L,y\R)) :- value(V).
exists_step(case(inr(V),x\L,y\R)) :- value(V).
